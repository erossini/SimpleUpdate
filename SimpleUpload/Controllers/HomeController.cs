    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Post(List<IFormFile> files)
        {
            ...
            foreach (var formFile in files)
            {
                ...
                {
                    (uploadSuccess, uploadedUri) = await UploadToBlob(Path.GetFileName(formFile.FileName), null, stream);
        ...
        [HttpPost]
        public async Task<IActionResult> UploadMyFiles()
        {
            long size = 0;
            ...
            foreach (var file in files)
            {
                ...
                {
                    (uploadSuccess, uploadedUri) = await UploadToBlob(Path.GetFileName(file.FileName), null, stream);
                    TempData["uploadedUri"] = uploadedUri;
                }
            }
        ...
        private async Task<(bool, string)> UploadToBlob(string filename, byte[] imageBuffer = null, Stream stream = null)
        {
            ...
            string storageConnectionString = "DefaultEndpointsProtocol=https;AccountName=explorerteststorage;AccountKey=YqiP5e0rRG5oEKzgRTF3WQ2iclRVaa1gmXVzBF9CWLkED6ijzE3N8iES+qPiRBzg/CjYKrlvimbu+hAIxxKXtw==;EndpointSuffix=core.windows.net"; // _configuration["storageconnectionstring"];
            ...
            if (CloudStorageAccount.TryParse(storageConnectionString, out storageAccount))
            {
                try
                {
                    ...
                    cloudBlobContainer = cloudBlobClient.GetContainerReference("uploads");
